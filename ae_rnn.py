...
from utils import snr_db2sigma, snr_sigma2db, errors
...
def get_args():
    ...
    parser.add_argument('-enc_act', choices=['tanh', 'linear'], default='linear')
    parser.add_argument('-loss', choices=['binary_crossentropy', 'mean_squared_error'], default='mean_squared_error')
    parser.add_argument('-block_noise_prob', type=float, default=1.0)
...
class AE():
    ...
    def build_channel(self, args):
        ...
        def channel(x):
            ...
            noise_sigma_pure =  tf.random_uniform(tf.shape(x),
                minval=noise_sigma_high,
                maxval=noise_sigma_low,
                dtype=tf.float32
            )

            select = K.random_binomial((args.batch_size,1), p=args.block_noise_prob)
            selection = tf.tile(select, [args.block_len, args.code_rate])
            selection = tf.reshape(selection, tf.shape(x))

            noise_sigma = tf.to_float(selection) * noise_sigma_pure
    ...
    def train(self, args):
        ...
        self.encoder.save_weights('./tmp/enc1_'+args.id+'.h5')
        self.decoder.save_weights('./tmp/dec1_'+args.id+'.h5')

        self.test(args,'./tmp/enc1_'+args.id+'.h5',  './tmp/dec1_'+args.id+'.h5')
